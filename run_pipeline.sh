...
cp -r $SOURCE_DIR/new_model_train.npz "$DEST_DIR/train.npz"
cp -r $SOURCE_DIR/new_model_val.npz "$DEST_DIR/val.npz"
cp -r $SOURCE_DIR/new_model_test.npz "$DEST_DIR/test.npz"
...
python GNN_model/dcrnn_train_pytorch.py \
...
python GNN_model/save_predictions.py \
    ...
    --output_filename "${OUTPUT_FOLDER}/predictions.npz" | tee "${OUTPUT_FOLDER}/prediction_log.txt"

#rm "$TEMP_CONFIG_FILE"

# Copy best weights into the output folder
cp "${LATEST_FILE}" "${OUTPUT_FOLDER}/model_weights.tar"
